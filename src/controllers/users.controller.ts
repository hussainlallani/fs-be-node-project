...
//   // Use .toObject() and manually remove fields
//   //Converts this document into a plain-old JavaScript object
//   const { password, __v, ...userObj } = savedUser.toObject();
//   return userObj as Omit<IUser, "password" | "__v">;
// }
...
/**
 * Create a new user and return the safe version (without password).
 */
export async function createUser(
  _id: string,
  data: IUser
): Promise<Omit<IUser, "__v">> {
  const salt = await bcrypt.genSalt(10);
  data.password = await bcrypt.hash(data.password, salt);
  const user = new UserModel(data);
  const savedUser = await user.save();
  const { password, __v, ...userObj } = savedUser.toObject();
  // return userObj as Omit<IUser, "__v">;
  return savedUser;
}
...
export async function getUserByEmailAndPassword(
  email: string,
  password: string
): Promise<Omit<IUser, "password" | "__v"> | null> {
  const user = await UserModel.findOne({ email }).exec();
  ...
  return safeUser as Omit<IUser, "password" | "__v">;
}
