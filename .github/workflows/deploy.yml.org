name: Deploy Node.js App to Contabo

on:
  push:
    branches:
      - main

jobs:
  deploy:
    runs-on: ubuntu-latest
    steps:
      - name: Checkout Repo
        uses: actions/checkout@v4

      - name: Deploy with Rsync
        uses: burnett01/rsync-deployments@6.0.0
        with:
          switches: -avz --delete
          path: ./
          remote_path: /var/www/fs-be-node-project
          remote_host: ${{ secrets.SSH_HOST }}
          remote_user: admin
          remote_key: ${{ secrets.SSH_PRIVATE_KEY }}
          remote_port: 2222

      - name: Setup & Run Node.js + Next.js Apps
        uses: appleboy/ssh-action@master
        with:
          host: ${{ secrets.SSH_HOST }}
          username: admin
          key: ${{ secrets.SSH_PRIVATE_KEY }}
          port: 2222
          script: |
            echo "🔧 Starting deploy on $(hostname)"
            # Ensure target directory exists and is owned by admin
            sudo mkdir -p /var/www/fs-be-node-project
            sudo chown -R admin:admin /var/www/fs-be-node-project

            cd /var/www/fs-be-node-project || {
            echo "❌ Failed to access project directory!"
            exit 1
            }

            echo "📁 Project directory contents:"
            ls -la

            # Node.js Setup
            if ! command -v node &> /dev/null; then
              echo "📥 Installing Node.js..."
              curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -
              sudo apt-get install -y nodejs
            fi

            # PM2 Setup
            if ! command -v pm2 &> /dev/null; then
              echo "📥 Installing PM2..."
              npm install -g pm2
            fi

            # Backend Setup
            echo "🧹 Cleaning backend build..."
            npm run clean || true
            echo "📦 Installing backend dependencies..."
            npm ci --production
            echo "🏗️ Building backend..."
            npm run build

            # MongoDB
            pm2 delete mongo_fs_be_node_project 2>/dev/null || true
            pm2 start "mongod --config /etc/mongod.conf" --name "mongo_fs_be_node_project"

            # Node.js App
            pm2 delete fs-be-node-app 2>/dev/null || true
            pm2 start dist/index.js --name "fs-be-node-app"

            # GameHub Next.js Setup
            echo "🚀 Deploying GameHub Next.js app..."
            cd gamehub || {
              echo "❌ GameHub directory not found!"
              exit 1
            }

            echo "📦 Installing GameHub dependencies..."
            npm install

            echo "🏗️ Building GameHub..."
            npm run build || {
            echo "❌ GameHub build failed!"
            exit 1
            }

            echo "📁 Checking GameHub build output:"
            ls -la .next || echo "⚠️ .next folder not found!"

            # Start Next.js app with PM2

            pm2 delete gamehub-next-app 2>/dev/null || true
            pm2 start "npm run start" --name "gamehub-next-app" --cwd /var/www/fs-be-node-project/gamehub

            # Save PM2 state
            pm2 save
            sudo pm2 startup systemd -u admin --hp /home/admin

            echo "🎉 Deployment completed successfully!"
